...
type Project struct {
	...
	Cost        float32
	Status      string `gorm:"not null; size:10 ;default:'draft'"`

	NIP        string
	KategoriID uint

	Kategori      Kategori `gorm:"constraint:OnUpdate:CASCADE,OnDelete:SET NULL;"`
	Peneliti      Peneliti `gorm:"constraint:OnUpdate:CASCADE,OnDelete:SET NULL;"`
...
func (u *Project) BeforeCreate(tx *gorm.DB) error {
	...
	if u.FundUntil.After(u.CreatedAt.Add(time.Hour * 24 * 30 * 3)) {
